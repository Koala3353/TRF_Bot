...
import static com.general_hello.commands.commands.Hangman.VirtualBot.state.*;
...
public class VirtualBot extends ListenerAdapter {
    ...
    public static final String START_COMMAND = "ignthangman-start";
    public static final String RESET_COMMAND = "ignthangman-reset";
    ...
    public void handleMessageEvent(MessageReceivedEvent event) {
        ...
        String words = event.getMessage().getContentRaw().replaceAll("\\s++", "");
        if (words.equals(RESET_COMMAND)) {
    ...
    private void checkGuild(MessageReceivedEvent event) {
        if(event.getChannelType() == ChannelType.TEXT && event.getGuild().getIdLong() != guild.getIdLong()) {
        ...
        if(event.getChannel() == gameChannel) {
            ...
            if (currentGame.isWon() || currentGame.isLost()) {
                ...
                if (currentGame.isWon()) {
                    DatabaseManager.INSTANCE.setCredits(event.getAuthor().getIdLong(), 3000);
                }
                reset();
            }
        }
    ...
    private void handleStart(MessageReceivedEvent event) {
        String firstWord = event.getMessage().getContentRaw().toLowerCase().replaceAll("\\s++", "");
